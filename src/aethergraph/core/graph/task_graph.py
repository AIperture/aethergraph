...
import inspect
from typing import Dict, List, Mapping, Optional, Any, Iterable, Union, TYPE_CHECKING
import uuid
...
from .graph_spec import TaskGraphSpec, GraphView
...
from .visualize import to_dot, visualize, ascii_overview 
from .utils import _status_label, _logic_label, _short
...
class TaskGraph:
    spec: TaskGraphSpec
    state: TaskGraphState = field(default_factory=TaskGraphState) # mutable state, including node states, patches   
    ...
    _by_alias: Dict[str, str] = field(default_factory=dict, init=False, repr=False) # alias -> node_id
    _by_logic: Dict[str, List[str]] = field(default_factory=dict, init=False, repr=False) # logic -> [node_id, ...]
    _by_label: Dict[str, List[str]] = field(default_factory=dict, init=False, repr=False) # label -> {node_id, ...}
    _by_name: Dict[str, List[str]] = field(default_factory=dict, init=False, repr=False) # display name -> [node_id, ...]: TODO: decided if we need this

    @classmethod
    def new_run(self, spec: TaskGraphSpec, *, run_id: Optional[str] = None, **kwargs) -> TaskGraph:
        """ Create a new TaskGraph instance for a new run. """
        run_id = run_id or str(uuid.uuid4())
        # initialize empty node states
        nodes = {nid: TaskNodeState() for nid in spec.nodes.keys()}
        state = TaskGraphState(run_id=run_id, nodes=nodes)
        graph = self.from_spec(spec, state=state, **kwargs)
        return graph

    @classmethod
    def from_spec(cls, spec: TaskGraphSpec, *, state: Optional[TaskGraphState] = None):
    ...
    def __post_init__(self):
        ...
        if not getattr(self.state, "nodes", None):
            self.state.nodes = {nid: TaskNodeState() for nid in self.spec.nodes} # GraphSpec.nodes is Dict[str, TaskNodeSpec]
    ...
    def _reindex(self): 
        ...
        for nid, node in self._runtime_nodes.items():
            ...
            logic_name = node.spec.tool_name or (node.spec.logic if isinstance(node.spec.logic, str) else _short(node.spec.logic)) 

            if alias: self._by_alias[alias] = nid
            for label in labels:
                self._by_label.setdefault(label, set()).add(nid)
            if logic_name: self._by_logic.setdefault(logic_name, []).append(nid)
            if display: self._by_name.setdefault(display, []).append(nid)
    ...
    def find_by_logic(self, logic_prefix: str, *, first: bool=False) -> List[str] | Optional[str]:
    ...
    def find_by_display(self, name_prefix: str, *, first: bool=False) -> List[str] | Optional[str]:
    ...
    def pick_one(self, selector: str) -> Optional[str]:
    ...
    def patch_add_dependency(self, node_id: str, dependency_id: str):
        ...
        patch = GraphPatch(op="add_dependency", payload={"node_id": node_id, "dependency_id": dependency_id})
        self.state.patches.append(patch)
        self.state.rev += 1
        awaitable = None
        for obs in self.observers:
    ...
    def list_nodes(self, exclude_internal=True) -> List[str]:
        ...
        return list(self.spec.nodes.keys()) if not exclude_internal else [
            nid for nid in self.spec.nodes.keys() if not nid.startswith("_")
        ]
    ...
    async def set_status(self, node_id: str, status: NodeStatus):
        ...
        raise NotImplementedError("set_status() is not implemented yet. Use set_node_status() instead.")

    async def set_outputs(self, node_id: str, outputs: Dict[str, Any]):
        """ Set the outputs of a node. """
        raise NotImplementedError("set_outputs() is not implemented yet. Use set_node_outputs() instead.")
    ...
    async def reset(self, node_ids: Optional[List[str]] = None, *, recursive=True, direction="forward", preserve_outputs: bool = False):
        """ 
        ...
        return {"status": "partial_reset", "graph_id": self.spec.graph_id, "nodes_reset": list(set(target_ids))}
    ...
    def diff(self, other: TaskGraph) -> Dict[str, Any]:
        ...
        for nid, node in other.spec.nodes.items():
            if nid not in self.spec.nodes:
                ...
                if (set(old_node.dependencies) != set(node.dependencies)) or (old_node.metadata != node.metadata):
    ...
    def declare_inputs(self, *, 
        ...
        required_spec = {k: ParamSpec() for k in (required or [])} # currently we don't support detailed param spec. Only names are used.
        optional_spec = {k: ParamSpec(default=v) for k, v in (optional or {}).items()}
        if required: self.spec.io.required.update(required_spec)
        if optional: self.spec.io.optional.update(optional_spec)

    def expose(self, name: str, value: Union[Ref, Any]) -> None:
    ...
    def io_signature(self, include_values: bool = False) -> Dict[str, Any]:
        ...
        if include_values:
            out["outputs"]["values"] = {
                n: resolve_binding(bindings.get(n), self.state.node_outputs)
                for n in names
            }
        return out
