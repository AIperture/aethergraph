...
def current_builder() -> Optional["GraphBuilder"]:
...
class GraphBuilder:
    ...
    def add_node(self, node_spec: TaskNodeSpec) -> str:
        if node_spec.node_id in self.spec.nodes:
            raise ValueError(f"Node ID '{node_spec.node_id}' already exists in graph '{self.spec.graph_id}'")
        self.spec.nodes[node_spec.node_id] = node_spec
        return self 

    def add_tool_node(self, *, node_id: str, logic: str, inputs: Dict,
                      expected_input_keys: Optional[Iterable[str]] = None,
                      expected_output_keys: Optional[Iterable[str]] = None,
                      after: Optional[Iterable[str]] = None,
                      inject: list[str] | None = None,
                      tool_name: str | None = None,
                      tool_version: str | None = None) -> "GraphBuilder":
    ...
    def ensure_inputs_node(self):
        ...
        if GRAPH_INPUTS_NODE_ID not in self.spec.nodes:
            self.spec.nodes[GRAPH_INPUTS_NODE_ID] = TaskNodeSpec(
                node_id=GRAPH_INPUTS_NODE_ID, type="inputs",
                logic=None, inputs={}, dependencies=[],
                expected_input_keys=[], expected_output_keys=[],
    ...
    # ---- ids and utils ----
    def next_id(self, logic_name: Optional[str] = None) -> str:
        """ Generate a unique node ID. """
        base = (logic_name or "node").rstrip("_")
        return f"{base}_{next(self._auto_counter)}_{uuid.uuid4().hex[:6]}"
    
    def _next_readable_id(self, logic_name: Optional[str] = None) -> str:
    ...
    def register_alias(self, alias: str, node_id: str):
        if alias in self._alias_index and self._alias_index[alias] != node_id:
            raise ValueError(f"Alias '{alias}' already registered for node '{self._alias_index[alias]}', cannot re-register for '{node_id}'")
