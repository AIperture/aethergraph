...
def to_dot(self, cfg: _VizConfig | None = None) -> str:
    ...
    L(f'  node  [fontname="{cfg.fontname}", fontsize={cfg.fontsize}, style="rounded,filled", shape=box];')
    ...
    if io_lines:
        ...
        L(f'  subgraph cluster_io {{ style=dashed; color="#cccccc"; label="";')
        L(f'    io_summary [shape=note, style="rounded,filled", fillcolor="#f7f7f7", label="{label}"];')
        L('  }')
    ...
    for node_id, node in self.spec.nodes.items():
        ...
        if cfg.show_logic and logic is not None:
            logic_s = (logic if isinstance(logic, str)
...
def visualize(
    ...
    if outfile and return_dot:
        ...
        log.info(f"DOT file written to: {dot_path}. View the graph at https://dreampuf.github.io/GraphvizOnline/")
    ...
    try:
        ...
        if outfile:
            path = src.render(outfile, view=view, cleanup=True) # This will also create outfile but without .dot extension? 
...
def ascii_overview(self) -> str:
    ...
    for node_id, node in self.spec.nodes.items():
        ...
        logic  = getattr(node, "logic", None)
        logic_s = (logic if isinstance(logic, str)
                   else getattr(logic, "__name__", type(logic).__name__)) if logic else ""
