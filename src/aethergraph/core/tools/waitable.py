...
from typing import Any, Callable, List, Dict, Optional, get_type_hints
import inspect 
...
def _is_coro_fn(obj: Callable) -> bool:
    ...
    return inspect.iscoroutinefunction(obj) or (
        hasattr(obj, "__call__") and inspect.iscoroutinefunction(obj.__call__) # async __call__
    )
...
def _infer_inputs_for_waitable(cls_or_inst) -> list[str]:
    ...
    for p in sig.parameters.values():
        ...
        if p.name in {"node","context","logger"}: continue
        keys.append(p.name)
    return keys
