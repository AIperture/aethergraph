...
class SQLiteVectorIndex(VectorIndex):
    ...
    def _get_or_set_corpus_dim(
        ...
        """
        Ensure that all vectors for a corpus share the same dimensionality.

        - If this is the first time we see corpus_id, we record `dim`.
        - If corpus_id already has a dim and it doesn't match, we raise.
        """
        cur = conn.cursor()
        cur.execute("SELECT dim FROM corpus_meta WHERE corpus_id=?", (corpus_id,))
        row = cur.fetchone()

        if row is None:
            # First time for this corpus -> insert dim
            cur.execute(
                "INSERT INTO corpus_meta(corpus_id, dim) VALUES(?, ?)",
                (corpus_id, dim),
            )
            return dim
        ...
        if stored_dim != dim:
            raise RuntimeError(
                f"Corpus '{corpus_id}' already has dim={stored_dim}, "
                f"cannot add vectors with dim={dim}. "
                "Use a new corpus_id or re-index with a single embedding model."
            )
        return stored_dim
    ...
    async def add(
        ...
        def _add_sync():
            conn = self._connect()
            try:
                # ---------- NEW: enforce a single dim per corpus ----------
                if not vectors:
                    return  # sanity; should match chunk_ids length anyway

                first_vec = np.asarray(vectors[0], dtype=np.float32)
                if first_vec.ndim != 1:
                    first_vec = first_vec.reshape(-1)
                dim = int(first_vec.shape[0])

                # Will insert dim for new corpus or check consistency for existing
                self._get_or_set_corpus_dim(conn, corpus_id, dim)
                # ---------------------------------------------------------
                ...
                for cid, vec, meta in zip(chunk_ids, vectors, metas, strict=True):
                    ...
                    # Optional: extra guard to catch weird cases per-row
                    if v.shape[0] != dim:
                        raise RuntimeError(
                            f"Embedding dimension mismatch for corpus '{corpus_id}': "
                            f"expected dim={dim}, got dim={v.shape[0]} for chunk_id={cid}"
                        )
    ...
    def _search_bruteforce_sync(
        ...
        for chunk_id, vec_bytes, norm, meta_json in rows:
            ...
            if v.shape[0] != q.shape[0]:
                raise RuntimeError(
                    f"Vector dimension mismatch in corpus '{corpus_id}': "
                    f"stored dim={v.shape[0]}, query dim={q.shape[0]}. "
                    "You probably switched embedding model; please re-index or use a new corpus_id."
                )
