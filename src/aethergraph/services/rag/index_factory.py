...
def create_vector_index(*, backend: str, index_path: str | None, dim: int | None, root: str = "./aethergraph_data/rag"):
    """
    ...
    if backend == "faiss":
        ...
        try:
            ...
            path = str(Path(index_path) / "faiss") if index_path is not None else _default_index_path(root, "faiss")
            return FAISSVectorIndex(path, dim=dim)
        except Exception as e:
            logger.warning("FAISS backend unavailable (%s); falling back to sqlite.", e, exc_info=False)
            backend = "sqlite"

    # sqlite (default)
    from .index.sqlite_index import SQLiteVectorIndex  
    path = str(Path(index_path) / "sqlite") if index_path is not None else _default_index_path(root, "sqlite")
