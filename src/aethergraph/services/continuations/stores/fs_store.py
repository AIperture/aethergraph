...
import os, json, re, hmac, hashlib, threading, asyncio
...
class FSContinuationStore:  # implements AsyncContinuationStore
    ...
    def _corr_dir(self, scheme: str, channel: str, thread: str, message: str) -> Path:
        return self.root / "index" / "corr" / self._safe(scheme) / self._safe(channel) / self._safe(thread) / self._safe(message)
    ...
    def _write_token_index(self, run_id: str, node_id: str, token: str) -> None:
        with self._lock:
            ...
            p.write_text(json.dumps({"run_id": run_id, "node_id": node_id}, indent=2), encoding="utf-8")
    ...
    async def bind_correlator(self, *, token: str, corr: Correlator) -> None:
        def _bind():
            ...
            if tokens_path.exists():
                try: toks = json.loads(tokens_path.read_text(encoding="utf-8"))
            ...
            if token not in toks:
                ...
                paths = []
                if r.exists():
                    try: paths = json.loads(r.read_text(encoding="utf-8"))
    ...
    async def last_open(self, *, channel: str, kind: str) -> Optional[Continuation]:
        ...
        def _scan():
            ...
            if not runs_path.exists(): return waits
    ...
    async def list_waits(self) -> list[Dict[str, Any]]:
        def _scan():
            out = []
            runs_path = self.root / "runs"
            if not runs_path.exists(): return out
            for run_dir in runs_path.iterdir():
                nodes_dir = run_dir / "nodes"
                if not nodes_dir.exists(): continue
                for node_dir in nodes_dir.iterdir():
