...
from aethergraph.contracts.services.memory import Distiller, HotLog, Persistence, Indices, Event
...
class EpisodeSummarizer(Distiller):
    ...
    """
    def __init__(self, *, include_metrics: bool = True, note_limit: int = 8, note_chars: int = 2000):
    ...
    async def distill(
        ...
        **kw
        ...
        events = await hotlog.recent(run_id, kinds=["tool_start","tool_result","error","run_summary"], limit=400)
        ...
        for e in eps:
            if e.event_id: srcs.append(e.event_id)
        ...
        eid = _stable_event_id({
            "ts": ts, "run_id": run_id,
            "tool": tool, "kind": "run_summary", "text": compact_text[:200]
        })
        evt = Event(
            event_id=eid, ts=ts, text=compact_text, metrics={"notes": len(notes)}, signal=0.5, **evt_base
        )
