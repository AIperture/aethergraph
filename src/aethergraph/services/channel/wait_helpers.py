...
async def create_and_notify_continuation(
    ...
    if corr:
        ...
        if peek:
            await store.bind_correlator(token=cont.token, corr=Correlator(peek.scheme, peek.channel, peek.thread, ""))
        else:
            await store.bind_correlator(token=cont.token, corr=Correlator(bus._prefix(ch_key), ch_key, "", ""))
