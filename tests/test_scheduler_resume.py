...
class _Store:
    def __init__(self, *, token: str = "token-1") -> None:
        self.continuation = SimpleNamespace(token=token)
...
async def test_resume_bus_dispatches_once_and_deletes_after_success() -> None:
    ...
    with pytest.raises(PermissionError, match="Invalid continuation"):
        await bus.enqueue_resume(
            run_id="run-1",
            node_id="node-1",
            token="token-1",
...
@pytest.mark.asyncio
async def test_resume_bus_rejects_invalid_token_without_dispatch() -> None:
    registry = SchedulerRegistry()
    scheduler = _Scheduler(asyncio.get_running_loop())
    registry.register("run-1", scheduler)
    store = _Store()
